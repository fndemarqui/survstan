...
ahreg <- function(formula, data, baseline = "weibull", dist = NULL, init = 0, ...){
  ...
  if(is.character(baseline)){
    ...
    if(baseline == "bernstein"){
      ...
      message("Piecewise exponential distribution not available for AH models. Using Bernstein polynomials instead!")
      baseline <- "bernstein"
      baseline <- get(baseline, mode = "function", envir = parent.frame())
    }
  }
  ...
  res <- reparametrization(fit, survreg = "ah", output$baseline, labels, tau, p, m)
